...
const getMetricUI = (key: string) => {
  ...
  if (k.includes('suhu') || k.includes('temp')) return { label: 'Suhu', icon: '­ƒîí´©Å', color: '#E65100', isProgress: false }
  if (k.includes('kelembapan') || k.includes('humid')) return { label: 'Kelembapan', icon: '­ƒÆº', color: '#1565C0', isProgress: true }
  if (k.includes('cahaya') || k.includes('light')) return { label: 'Intensitas Cahaya', icon: 'ÔÿÇ´©Å', color: '#F59E0B', isProgress: false }
  if (k.includes('air') || k.includes('water')) return { label: 'Level Air', icon: '­ƒîè', color: '#1565C0', isProgress: true }
  if (k.includes('pakan') || k.includes('feedlevel')) return { label: 'Sisa Pakan', icon: '­ƒî¥', color: '#F59E0B', isProgress: true }
  if (k.includes('jarak') || k.includes('feeddist')) return { label: 'Jarak Pakan', icon: '­ƒôÅ', color: '#6B7280', isProgress: false }
  if (k.includes('populasi')) return { label: 'Populasi', icon: '­ƒÉô', color: '#795548', isProgress: false }
  return { label: key, icon: '­ƒôè', color: '#6B7280', isProgress: false }
}
    ...
    <div className="sector-dash fade-up">
      <div className="sector-dash-header">
        ...
        <div>
          ...
          <p style={{ margin: '2px 0 0', fontSize: 14, color: 'var(--text-secondary)' }}>{sector.unit} ÔÇö Monitoring Real-time</p>
        </div>
            ...
            {isKandang ? (
              <>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', padding: '12px', background: 'var(--bg-base)', borderRadius: 8, gap: 12, border: '1px solid var(--border-color)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontSize: 18 }}>ÔÜÖ´©Å</span>
                        <div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', padding: '12px', background: 'var(--bg-base)', borderRadius: 8, gap: 12, border: '1px solid var(--border-color)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontSize: 18 }}>­ƒî¥</span>
                        <div>
              ...
              <div key={ctrl.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', background: 'var(--bg-base)', borderRadius: 12, border: '1px solid var(--border-color)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ fontSize: 20 }}>­ƒöä</span>
                  <div>
        ...
        <div className="modal-overlay" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="card fade-up" style={{ width: '100%', maxWidth: 500, padding: 24, margin: 20, position: 'relative' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
              <div style={{ width: 40, height: 40, borderRadius: 12, background: 'linear-gradient(135deg, #8B5CF6, #6D28D9)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 20 }}>Ô£¿</div>
              <div>
